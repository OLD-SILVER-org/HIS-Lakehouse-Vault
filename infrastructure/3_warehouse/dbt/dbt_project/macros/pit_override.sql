...
{#- Setting ghost values to replace NULLS (Hex strings instead of BYTEA) -#}
...
backfill AS (
    ...
    {%- for sat_name in satellites -%}
        ...
        COALESCE(MAX({{ sat_name | lower ~ '_src' }}.{{ sat_pk }}), '{{ ghost_pk }}')
        AS {{ sat_name }}_{{ sat_pk_name }},

        COALESCE(MAX({{ sat_name | lower ~ '_src' }}.{{ sat_ldts }}), CAST('{{ ghost_date }}' AS TIMESTAMP))
